...
class DynamicOwner(onAccessAfterKilled: () => Unit) {
  ...
  /** Current owner reference - null when Inactive */
  private var _currentOwner: OneTimeOwner = _
  ...
  @inline def maybeCurrentOwner: Option[Owner] =
    if (_currentOwner == null) None else Some(_currentOwner)
    ...
    state match {
      case Inactive =>
        Transaction.onStart.shared {
          _currentOwner = new OneTimeOwner(onAccessAfterKilled)
          _numPrependedSubs = 0
          ...
          while (i < originalNumSubs) {
            ...
            subscriptions(ix).onActivate(_currentOwner)
            i += 1
          }
  ...
  def deactivate(): Unit = {
    state match {
      case Active =>
        ...
        _currentOwner._killSubscriptions()
        removePendingSubscriptionsNow()
  ...
  private[ownership] def addSubscription(subscription: DynamicSubscription, prepend: Boolean): Unit = {
    ...
    if (_currentOwner != null) {
      subscription.onActivate(_currentOwner)
    }
  }
