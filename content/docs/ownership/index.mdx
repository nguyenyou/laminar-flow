...
classDiagram
    ...
    class Owner {
        ...
        #onOwned(subscription: Subscription) void
        ~onKilledExternally(subscription: Subscription) void
        ~own(subscription: Subscription) void
    }
    
    class OneTimeOwner {
        -_isKilledForever: Boolean
        -onAccessAfterKilled: () => Unit
        +isKilledForever: Boolean
        ~own(subscription: Subscription) void
        #killSubscriptions() void
    }
    ...
    class DynamicOwner {
        ...
        -isSafeToRemoveSubscription: Boolean
        -pendingSubscriptionRemovals: JsArray~DynamicSubscription~
        -_maybeCurrentOwner: Option~Owner~
        -numPrependedSubs: Int
        -onAccessAfterKilled: () => Unit
        +maybeCurrentOwner: Option~Owner~
        +isActive: Boolean
        +hasSubscriptions: Boolean
        +numSubscriptions: Int
        +activate() void
        +deactivate() void
        ~addSubscription(subscription: DynamicSubscription, prepend: Boolean) void
        ~removeSubscription(subscription: DynamicSubscription) void
    }
    ...
    class DynamicSubscription {
        ...
        +isOwnedBy(owner: DynamicOwner) Boolean
        +kill() void
        ~onActivate(owner: Owner) void
        ~onDeactivate() void
    }
    
    class DynamicSubscription_Companion {
        <<object>>
        +unsafe(dynamicOwner, activate, prepend) DynamicSubscription
        +subscribeCallback(dynamicOwner, activate, prepend) DynamicSubscription
        +subscribeObserver(dynamicOwner, observable, observer) DynamicSubscription
        +subscribeSink(dynamicOwner, observable, sink) DynamicSubscription
        +subscribeFn(dynamicOwner, observable, onNext) DynamicSubscription
        +subscribeBus(dynamicOwner, eventStream, writeBus) DynamicSubscription
    }
    ...
    class TransferableSubscription {
        ...
        +setOwner(nextOwner: DynamicOwner) void
        +clearOwner() void
    }
    
    Owner <|-- OneTimeOwner : extends
    Owner <|-- ManualOwner : extends
    
    Owner "1" *-- "*" Subscription : owns
    DynamicOwner "1" *-- "*" DynamicSubscription : owns
    DynamicOwner ..> OneTimeOwner : creates
    
    DynamicSubscription ..> Subscription : creates on activate
    DynamicSubscription --> DynamicOwner : belongs to
    
    TransferableSubscription ..> DynamicSubscription : creates internally
    TransferableSubscription --> DynamicOwner : can transfer between
    
    DynamicSubscription_Companion ..> DynamicSubscription : creates
```
